...
var saveReceivedSMS = function(_id_sparrow,_from,_text,_to,callback)
{
	var currentDate=new Date().toJSON();
	var smsToAdd=new Sms({id:null,id_sparrow:_id_sparrow,direction:"in",from:_from,text:_text,to:_to,status:"0",dateSent:currentDate,dateLog:null});
	var requestResult=smsToAdd.save(function(err,result){
		if(err) return handleError(err);
		if(result!==null) 
		{
