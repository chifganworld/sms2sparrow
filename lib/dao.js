...
var deleteSMSCollection	= function(){
	var exchangeAPI =require ("./api");
	var MongoClient = require('mongodb').MongoClient;
	var url="mongodb://localhost:27017/sms2db";
	MongoClient.connect(url, function(err, db) {
		if (err) throw handleError(err);
		db.collection("sms").drop(function(err, delOK) {
			if (err) throw err;
...
{
	var currentDate=new Date().toJSON();
	var requestResult=Sms.update({"id":idToEdit,"dateSent":null},{$set:{"status":"1","dateSent":currentDate}},function(error,res){
		if(error)
		{
			console.error(error);
			return callback(-1);
...
var editReceivedSMS=function(idToEdit,callback)
{
	var currentDate=new Date().toJSON();
	var requestResult=Sms.update({"id_sparrow":idToEdit},{$set:{"status":"1","dateLog":currentDate}},function(error,res){
		if(error)
		{
			console.error(error);
			return callback(-1);
