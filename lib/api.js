...
if(manifest.useSMSAPISimulator=="on")
{
	useSMSAPISimulator=true;
}
else
{
	useSMSAPISimulator=false;
}
...
{
	var urlRequest="";
	if(!useSMSAPISimulator)
	{
		urlRequest=`${URLSPARROW}/?&token=${manifest.activities.SMSAPI.token}&from=${from}&to=${to}&text=${text}`;
	}
	else
	{
		urlRequest=`${URLSPARROW}/token=${manifest.activities.SMSAPI.token}&from=${from}&to=${to}&text=${text}`;
...
exports.notifyReceivedSMS=function notifyReceivedSMS(from,text,date,callback)
{
	...
	var urlRequest=`${URLSecureRP}/received/${uid}/?from=${from}&text=${escapedText}&date=${date}`;
	var now=new Date();
	var dateLog=now.toISOString();
